...
<style scoped>
.board { display: grid; gap: 3px; padding: 12px; background: #2d6a4f; border-radius: 12px; box-shadow: 0 8px 32px rgba(0,0,0,0.4); }
.cell { width: 52px; height: 52px; background: #40916c; border-radius: 4px; display: flex; align-items: center; justify-content: center; cursor: pointer; position: relative; transition: background 0.15s; }
.cell:hover { background: #52b788; }
.cell.disabled { cursor: default; pointer-events: none; }
.cell.disabled:hover { background: #40916c; }
.coord-label { display: flex; align-items: center; justify-content: center; color: rgba(255,255,255,0.5); font-size: 0.75rem; font-weight: 600; user-select: none; pointer-events: none; }
.disc { width: 76%; height: 76%; border-radius: 50%; position: absolute; transition: transform 0.35s ease; }
.disc.black { background: radial-gradient(circle at 35% 35%, #555, #111); box-shadow: 2px 2px 6px rgba(0,0,0,0.5); }
.disc.white { background: radial-gradient(circle at 35% 35%, #fff, #ccc); box-shadow: 2px 2px 6px rgba(0,0,0,0.3); }
...
.hint { width: 30%; height: 30%; border-radius: 50%; background: rgba(0,0,0,0.2); position: absolute; pointer-events: none; }
.hint.on { background: #ff4444; box-shadow: 0 0 8px rgba(255,68,68,0.6); animation: pulse 1.2s ease-in-out infinite; }
@keyframes pulse { 0%, 100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.3); opacity: 0.7; } }
.board-placeholder { min-width: 320px; min-height: 200px; display: flex; align-items: center; justify-content: center; color: #aaa; }
</style>
